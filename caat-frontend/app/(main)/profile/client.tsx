...
export default function ProfileClient({
  ...
  /** Profile row resolved on the server (C8); when present the page paints
   *  real content on first render instead of a skeleton. Null for a brand-new
   *  user with no row yet, in which case the client loads (and creates) it. */
  initialProfile?: ProfileRow | null;
  initialScores?: StandardisedTestScore[];
  initialMajorOptions?: string[];
}) {
  const hasInitialProfile = initialProfile != null;
  const [profile, setProfile] = useState<ProfileRow | null>(initialProfile ?? null);
  const [scores, setScores] = useState<StandardisedTestScore[]>(initialScores ?? []);
  const [majorOptions, setMajorOptions] = useState<string[]>(initialMajorOptions ?? []);
  const [loading, setLoading] = useState(!hasInitialProfile);
  const [loadError, setLoadError] = useState(false);

  useEffect(() => {
    // Server already resolved the profile (and its scores/majors) — skip the
    // client load. For a brand-new user (no row yet) the server returns null,
    // so fall through to the client fetch, which creates the row.
